           ...
           <div class="cols" v-else>
             <div v-for="(col, col_index) in cols.ColmnList" :key="col_index">
               ...
               <div
                 ...
                 :style="[{ background: '#F2F4F7' }, { marginLeft: `0.52vw` }, { marginTop: `0.52vw` }]"
               >
                 <div class="rooms">
                   <div
                     ...
                     :key="index"
                     class="room-item"
                     :class="{ 'dimmed': !isBedVisible(item) }"
...
const isBedVisible = (item) => {
  const filters = globalStore.bedStatusFilter;
  if (!filters || filters.length === 0) {
    return true;
  }
  return filters.some(filter => {
    if (filter === 'Occupied') return item.BedStatus === 1;
    if (filter === 'Available') return item.BedStatus === 2;
    if (filter === 'Discharging') return item.BedStatus === 3;
    if (filter === 'Housekeeping') return item.BedStatus === 4;
    if (filter === 'Blocked') return item.BedStatus === 5;
    if (filter === 'Isolation') return item.IsolationBedType !== null && item.IsolationBedType !== undefined;
    if (filter === 'BookingBed') return item.IsBooking === true;
    return false;
  });
};
